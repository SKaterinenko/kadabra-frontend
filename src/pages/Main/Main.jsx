...
import { cleanCategory, getCategory, getCategoryList } from '../../redux/categoryReducer';
...
const Main = () => {
    ...
    return (
        <>
            <Header />
            <section className='mainBanner' style={{ backgroundImage: `url(${previewImage})` }}>
            </section>
            <main className='container'>
                <section className='monthly-best-section'>
                    <div className='header-action-sm'>
                        <h1 className='header-action-name'>{t("bestOfMonth")}</h1>
                        <Link to='/best' className='header-action-link'>{t("main:seeAll")}</Link>
                    </div>
                    <div className='carousel-wrapper'>
                        <Carousel gridOnSm noPriceCards items={monthlyBest ? monthlyBest.products : null} centered />
                    </div>
                </section>
                {!login && (
                    <section className='welcome-section'>
                        <div className='d-flex flex-column flex-items-center welcome-box'>
                            <div className=' d-flex flex-column flex-items-center welcome-box-inner'>
                                <div className='d-flex'>
                                    <svg viewBox="0 0 85 85" fill="none"
                                        ...
                                        <path
                                            ...
                                            stroke="#241E3C" strokeWidth="5" strokeLinecap="round"
                                ...
                                <p>{t("main:purchases")}</p>
                                <div className='d-flex flex-row w-100 welcome-box-actions'>
                                    <Button to='/signup' text={t("registration")} color='pink' bold fill variant='lg' />
                                    <Button to='/login' text={t("login")} color='pink' bold variant='lg' />
                                </div>
                            </div>
                        </div>
                        <div className='welcome-banner' style={{ backgroundImage: `url(${mainWelcomeBanner})` }} />
                    </section>
                )}
                <section className='categories-section'>
                    <div className='header-action-sm'>
                        <h1 className='header-action-name'>{t("categories")}</h1>
                        {/* <Link to='/category' className='header-action-link'>Смотреть все</Link> */}
                    </div>
                    <CategoryList />
                </section>

                <section className='d-flex flex-items-center kadabra-desc'>
                    <div className='w-100 shadow desc'>
                        <div className="logo">
                            <img src={logo} className='user-select-none user-drag-none' alt="Logo" />
                        </div>
                        <div className="icons">
                            <div className="price">
                                <img src={price} alt="Price" className='user-select-none user-drag-none' />
                                <p>{t("main:prices")}</p>
                            </div>
                            <div className="delivery">
                                <img src={delivery} alt="Delivery" className='user-select-none user-drag-none' />
                                <p>{t("main:delivery")}</p>
                            </div>
                            <div className="quality">
                                <img src={quality} alt="Quality" className='user-select-none user-drag-none' />
                                <p>{t("main:quality")}</p>
                            </div>
                            <div className="pay">
                                <img src={pay} alt="Pay" className='user-select-none user-drag-none' />
                                <p>{t("main:pay")}</p>
                            </div>
                        </div>
                    </div>
                    {size.width > 1150 && <div className='d-flex bg-dark-blue br-5px telegram'>
                        <h1>{t("main:telegram")}</h1>
                        <Button to={{ pathname: 'https://t.me/uzAmazonDelivery' }} target="_blank" color='grey'>
                        ...
                        <h1>{t("main:telegramDesc")} <br /> {t("main:telegramDesc2")}</h1>
                    </div>
                    }
                </section>
                {login &&
                    <section className='mt-60px login-carousel'>
                        <div className='header-action-sm mb-10px'>
                            <h1>{t("likeProducts")}</h1>
                            <Link to='/category' className='header-action-link'>{t("main:seeAll")}</Link>
                        </div>
                        <div className='carousel-wrapper'>
                ...
                {size.width >= 768 &&
                    <section className='banner-3' style={{ backgroundImage: `url(${mainBottomBanner})` }} />
                }
                <section className='carousel-section'>
                    ...
                    <div className='carousel-wrapper'>
                        <Carousel noPriceCards centered={false} sm
                            items={carouselBottom ? carouselBottom.products : null} />
                    </div>
                </section>
            </main>
            <Footer />
